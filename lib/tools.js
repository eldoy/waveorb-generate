...
tools.copyFolder = function(from, to, name, fn) {
  tools.mkdir(to)
  fs.readdirSync(from).forEach(function(item) {
    ...
    if (fs.lstatSync(afrom).isFile()) {
      const check = fn ? fn(ato) : ato
      if (!tools.exist(check)) {
        tools.write(ato, tools.convert(afrom, name))
      } else if (check.endsWith('/en.yml')) {
        // Append new entries to language file
        tools.append(ato, '\n')
        tools.append(ato, tools.convert(afrom, name))
      }
    } else {
      tools.copyFolder(afrom, ato, name, fn)
    }
  })
}
