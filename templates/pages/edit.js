          ...
          if (type == 'text') {
            ...
            if (options.length) {
              return options.map(o => {
                return `<label><input type="radio" name="${type}" value="${o}"\${item.${k} == '${o}' ? ' checked' : ''}> ${cap(o)}<label>`
              }).join('\n        ')
            }
            return ''
            ...
            if (options.length) {
              return options.map(o => {
                return `<label><input type="checkbox" name="${type}" value="${o}"\${item.${k}.includes('${o}') ? ' checked' : ''}> ${cap(o)}<label>`
              }).join('\n        ')
            }
            return ''
          } else {
