          ...
          if (type == 'text') {
            ...
            if (options.length) {
              return options.map(o => {
                return `<label>${cap(o)} <input type="radio" name="${type}" value="${o}"\${item.${k} == '${o}' ? ' checked' : ''}><label>`
              }).join('\n        ')
            }
            return ''
            ...
            if (options.length) {
              return options.map(o => {
                return `<label>${cap(o)} <input type="checkbox" name="${type}" value="${o}"\${item.${k}.includes('${o}') ? ' checked' : ''}><label>`
              }).join('\n        ')
            }
            return ''
          } else {
